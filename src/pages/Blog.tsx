/**
 * Blog page for Ananta Yoga & Meditation.
 * Displays a curated selection of reflective articles with links to full posts.
 */
            ...
            {blogsData.posts.map((post) => (
              ...
              >
                <header>
                  ...
                  <h2 className="mt-2 text-sm font-semibold uppercase tracking-[0.16em] text-[#2A441F]">
                    {post.title[lang]}
                  </h2>
                </header>
                ...
                <div className="mt-4 flex items-center justify-between">
                  <p className="text-[0.7rem] text-[#4A5C35]">
                    {post.readingTime[lang]} · {t('blog.cadenceNote')}
                  </p>
                  <Link
                    to={`/blog/${post.slug}`}
                    className="rounded-full bg-[#2A441F] px-4 py-1.5 text-[0.65rem] font-medium uppercase tracking-[0.2em] text-[#F5F1E8] transition-colors hover:bg-[#1A1508]"
                  >
                    {lang === 'es' ? 'Leer' : 'Read'}
                  </Link>
                </div>
